...
const alreadyUploaded = photos.filter((photo) => photo.url);
const filterPhotos = photos.filter((photo) => !photo.url);
const urlPhotos = ['123', "234"]
let res
if (urlPhotos.every((url) => url !== undefined)) {
    res = [...alreadyUploaded, ...urlPhotos.map((url) => ({ url }))]
    // console.log(res)
    if (res.every((item) => item?.url && typeof item?.url === 'string')) {
        // console.log("url")
    }
}
const amenities = [
...
console.log(amenities.toString())
