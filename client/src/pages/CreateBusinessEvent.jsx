...
function CreateBusinessEvent() {
    ...
    useEffect(() => {
        dispatch(setEvent({ type: "business" }))
    }, [])
            ...
            if (urlPhotos.every((url) => url !== undefined)) {
                ...
                if (updatedEvent?.photos?.every((item) => item?.url && typeof item?.url === 'string')) {
                    if (updatedEvent?._id) {
                        ...
                        const result = await createEvent({ ...updatedEvent, type: "business" }).unwrap();
                        console.log(result);
    ...
    useEffect(() => {
        if (progress > 0 && progress < 4) {
            ...
            navigate(`/create-event/create-business-event/${progress}?eventId=${eventCreated ? eventCreated._id : eventId}&status=draft`);
        }
    }, [progress]);
