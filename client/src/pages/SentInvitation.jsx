...
import { useGetAllSentInvitationsQuery } from "../api/api"
import { toast } from 'react-toastify'
import { capitalize, formatTime, getDate, getFilterNotification, getFullDay, getFullTime, getTime } from '../utils/customUtility'

function SentInvitation() {
    const { data, isSuccess, error } = useGetAllSentInvitationsQuery();
                ...
                {sentInvitation && sentInvitation.length > 0 && sentInvitation.map((item, i) => (
                    <React.Fragment key={item._id}>
                        ...
                        <div className='bg-black-light p-5 rounded-md space-y-2'>
                            ...
                            {item.isAccepted || item.pending ? (
                                <div className='grid grid-cols-2'>
                                    <div className='flex items-center col-span-1 gap-2'>
                                        <Button
                                            text="Cancel Invitation"
                                            className="bg-red hover:bg-red/80 h-8 flex items-center"
                                        />
                                    </div>
